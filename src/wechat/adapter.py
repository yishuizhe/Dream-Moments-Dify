...
class WxAuto4PollingAdapter:
    ...
    def send_voice(self, chat_name: str, file_path: str | os.PathLike[str]) -> tuple[Any, bool]:
        """Send a native voice message when the wxauto build exposes it.

        wxauto4 41.1.2 currently exposes only SendFiles. Since a file attachment
        is not a voice bar, unsupported adapters return ``(None, False)``.
        """
        path = str(Path(file_path).expanduser().resolve())
        if not os.path.isfile(path):
            raise FileNotFoundError(path)
        with self._ui_lock:
            for name in ("SendVoice", "SendAudio", "send_voice", "send_audio"):
                method = getattr(self.client, name, None)
                if not callable(method):
                    continue
                try:
                    return method(filepath=path, who=chat_name), True
                except TypeError:
                    return method(path, chat_name), True
            # SendFiles creates a file attachment, which is intentionally not
            # used as a substitute for a WeChat voice bar.
            return None, False
    ...
    def SendVoice(self, filepath: str, who: str, **_: Any) -> Any:  # noqa: N802
        return self.send_voice(who, filepath)[0]
    ...
    def poll_once(self) -> list[IncomingMessage]:
        ...
        for name in list(getattr(self, "always_poll", []) or []):
            if name in self.contacts and name not in contacts_to_read:
                contacts_to_read.insert(0, name)
        ...
        for contact in contacts_to_read:
            if self._contact_retry_after.get(contact, 0.0) > now:
                logger.debug("Skipping temporarily unavailable WeChat chat: %s", contact)
                continue
            try:
                ...
                if previous is None:
                    ...
                    unseen = [item for item in snapshot if item.token not in prev_set]
                    if previous and tokens and overlap == 0:
                        # wxauto may replace the complete visible message window
                        # while switching chats. There is no reliable ordering
                        # signal in that case, so treating the whole window as
                        # new can replay old @mentions and make the bot speak
                        # without a fresh request. Rebuild the baseline instead.
                        new_items = []
                        logger.warning("Chat %s lost message continuity; baseline rebuilt", contact)
                    else:
                        sequential = snapshot[overlap:]
                        new_items = sequential if sequential else unseen

                self._snapshots[contact] = tokens[-self.history_size :]
                self._contact_failures.pop(contact, None)
                self._contact_retry_after.pop(contact, None)
                changed = True
                result.extend(
                    item.message
                ...
                failures = self._contact_failures.get(contact, 0) + 1
                self._contact_failures[contact] = failures
                self._contact_retry_after[contact] = now + min(300.0, 5.0 * (2 ** min(failures - 1, 6)))
                if self._requires_client_reconnect(exc):
                    logger.warning("WeChat UI connection became unavailable; reconnecting before retry")
                    self.reconnect()
