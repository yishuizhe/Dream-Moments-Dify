...
from config.settings import (
    DIFY_API_KEY, DIFY_BASE_URL, LISTEN_LIST, DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL,
    ...
    def handle_wxauto_message(self, msg, chatName, is_group=False):
        try:
            ...
            if is_group and self.robot_name and content:
                if hasattr(msg, "handled") and msg.handled:
                    logger.warning("检测到重复处理的消息，跳过递归调用")
                    return

                logger.info(f"处理群聊@消息 - 机器人名称: {self.robot_name}")
                original_content = content

                # 移除@机器人后的内容，包括空格，确保robot_name安全
                content = re.sub(rf'@{re.escape(self.robot_name)}\s*', '', content).strip()
                logger.info(f"移除@后的消息内容: {content}")

                # 日志检查是否真正移除了@
                logger.debug(f"原始内容: {original_content}, 处理后内容: {content}")

                # 只要original_content中包含@机器人，说明需要处理
                if f'@{self.robot_name}' in original_content or original_content.startswith(self.robot_name):
                    if content == original_content:
                        logger.warning("可能存在@去除失败的情况，跳过递归调用")
                    else:
                        msg.handled = True  # 标记该消息已被处理，防止递归
                        chat_bot.handle_wxauto_message(msg, msg.sender, is_group=True)
                else:
            ...
            for chat in msgs:
                ...
                for msg in one_msgs:
                    try:
                        ...
                        # elif ROBOT_WX_NAME != '' and bool(re.search(f'@{ROBOT_WX_NAME}\s', msg.content)): 
