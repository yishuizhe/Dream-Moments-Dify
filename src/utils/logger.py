...
class LoggerConfig:
    ...
    def setup_logger(self, name: str = None, level: int = logging.INFO):
        ...
        # 如果已经有处理器，不重复添加
        if logger.handlers:
            self._ensure_root_file_logging(level)
            return logger
        ...
        # 同时给 root 挂文件日志，这样 handlers/services 的日志也能落盘
        self._ensure_root_file_logging(level)
        return logger

    @staticmethod
    def _quiet_protocol_loggers() -> None:
        """Keep request bodies and conversation prompts out of ordinary logs."""

        for logger_name in ("openai", "openai._base_client", "httpx", "httpcore"):
            logging.getLogger(logger_name).setLevel(logging.WARNING)

    def _ensure_root_file_logging(self, level: int = logging.INFO) -> None:
        root = logging.getLogger()
        root.setLevel(level)
        log_file = os.path.abspath(self.get_log_file())
        for handler in root.handlers:
            if isinstance(handler, RotatingFileHandler):
                try:
                    if os.path.abspath(getattr(handler, "baseFilename", "")) == log_file:
                        return
                except Exception:
                    pass
        file_handler = RotatingFileHandler(
