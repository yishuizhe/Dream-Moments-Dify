...
class ReplyFormatterTests(unittest.TestCase):
    ...
    def test_summary_never_sends_a_header_without_its_body(self):
        reply = "【今日概览】\n" + "主要聊了网络稳定性和工作安排。" * 8
        bubbles = split_summary_bubbles(reply, max_chars=40)
        self.assertTrue(bubbles[0].startswith("【今日概览】\n"))
        self.assertFalse(any(item == "【今日概览】" for item in bubbles))
        self.assertTrue(all(len(item) <= 40 for item in bubbles))
