...
class StartupCompatibilityTests(unittest.TestCase):
    ...
    def test_unrequested_group_image_does_not_download_or_trigger_ai(self):
        import main
        ...
        class FakeMessageHandler:
            def add_to_queue(self, **kwargs):
                raise AssertionError("unrequested group image must not reach the LLM")

        bot = main.ChatBot(
            FakeMessageHandler(), object(), FakeWeChat(), plugin_manager=FakePluginManager()
        )
        with patch.object(main, "download_wechat_image", side_effect=AssertionError("must not download")):
            bot.handle_wxauto_message(FakeMessage(), "测试群", is_group=True)

        self.assertEqual(bot.user_queues, {})

    def test_name_inside_sentence_triggers_group_ai(self):
        import main
        ...
        key = "测试群::member::群成员"
        self.assertIn(key, bot.user_queues)
        self.assertIn("麻烦说一下", bot.user_queues[key]["messages"][0])

    def test_requested_image_can_follow_the_trigger_message(self):
        import main
        ...
        class FakeVision:
            def recognize_image(self, path, is_emoji=False):
                return "发送了图片：一只橘猫"

        with patch.object(main.threading, "Timer", FakeTimer), patch.object(
            main, "download_wechat_image", return_value=main.__file__
        ) as download:
            bot = main.ChatBot(
                FakeMessageHandler(), FakeVision(), FakeWeChat(), plugin_manager=FakePluginManager()
            )
            bot.handle_wxauto_message(TextRequest(), "测试群", is_group=True)
            self.assertNotIn("测试群::member::群成员", bot.user_queues)
            bot.handle_wxauto_message(ImageRequest(), "测试群", is_group=True)

        key = "测试群::member::群成员"
        self.assertTrue(download.called)
        self.assertFalse(bot._has_pending_visual_request(key))
        self.assertIn("发送了图片：一只橘猫", bot.user_queues[key]["messages"][-1])
    ...
    def test_quote_reply_to_bot_triggers_group_ai_without_at(self):
        ...
        key = "测试群::member::群成员"
        self.assertIn(key, bot.user_queues)
        self.assertIn("继续说", bot.user_queues[key]["messages"][0])
    ...
    def test_random_auto_message_scheduler_was_removed(self):
        import main

        self.assertFalse(hasattr(main, "auto_send_message"))
        self.assertFalse(hasattr(main, "start_countdown"))
