...
> 公开发行版不包含 API Key、微信昵称、联系人名单、私聊或群聊记录、私人角色设定、运行日志、截图和本地状态。首次运行时会从空白模板创建本地配置。
...
Dream 会在启动时扫描项目根目录下的 `plugins/*/dream_plugin.py`。第三方插件、插件配置和插件数据库均不会随本仓库发布；安装前请自行审阅其代码、许可证和数据处理方式。
